...
export default function App() {
  ...
  const [matchPrefill, setMatchPrefill] = useState<{ league?: string; map?: string; date?: string; matchCode?: string } | null>(null);
  ...
  const handleEnterScheduleResults = (schedule: ScheduleEntry) => {
    ...
    const dateStr = scheduledDate && !Number.isNaN(scheduledDate.getTime())
  ...
  const handleViewScheduledMatch = (schedule: ScheduleEntry) => {
    const dateStr = schedule.scheduledAt ? schedule.scheduledAt.slice(0, 10) : "";
  ...
  const handleSaveMatch = async (matchData: Match) => {
    ...
    // result, auto-marks its corresponding schedule entry Finished so the admin doesn't have to
    // separately go find and click it themselves - whether that's the one explicitly linked via
    // "Enter Match Result" (scheduleToFinishOnSaveId), or - if the result was entered directly
    // instead, e.g. via the same-form "hasn't happened yet" toggle turned off - any other
    // not-yet-finished schedule entry for the same league and date, so it doesn't linger looking
    // "ongoing" forever just because this particular save didn't go through that button.
    const finalizeSuccess = () => {
      ...
      if (!isEdit) {
        const linkedSchedule = scheduleToFinishOnSaveId
          ...
          : schedules.find(s => !s.isFinished && s.league === matchData.league && s.scheduledAt && s.scheduledAt.slice(0, 10) === matchData.date);
  ...
  const handleDeleteMatch = async (id: string) => {
    ...
    const matchingSchedule = deletedMatch
      ? schedules.find(s => s.league === deletedMatch.league && s.scheduledAt && s.scheduledAt.slice(0, 10) === deletedMatch.date)
      : undefined;

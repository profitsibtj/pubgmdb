...
import { 
  Activity, Users, Swords, ArrowRightLeft, PlusCircle, LogOut, Sun, Moon, Lock, Award, Trophy
} from "lucide-react";
...
export default function App() {
  ...
  // Click outside admin dropdown to close it
  useEffect(() => {
  ...
  // Aggregate general stats banner at top
  const statsBanner = useMemo(() => {
    let totalWWCDs = 0;
    let totalElims = 0;
    const teamCounts = new Set<string>();

    const nonDailyMatches = matches.filter(m => !m.isDailyStats);

    nonDailyMatches.forEach((m) => {
      (m.teams || []).forEach((t) => {
        teamCounts.add(t.name);
        totalElims += t.eliminationPoints || 0;
        if (t.placement === 1) totalWWCDs += 1;
      });
    });

    return {
      matchesCount: nonDailyMatches.length,
      wwcdCount: totalWWCDs,
      elimsCount: totalElims,
          ...
          <div className={`mb-8 p-6 rounded-3xl border transition-all ${
            ...
            <div className="flex flex-col lg:flex-row gap-5 items-start justify-between">
              <div className="space-y-2 flex-1">
                ...
                <div className="mt-4 relative bg-slate-950 p-4 rounded-xl border border-slate-855 font-mono text-[11px] text-slate-300 max-h-48 overflow-y-auto whitespace-pre select-all">
...
CREATE POLICY "Allow public delete" ON matches FOR DELETE TO public USING (true);`}
              ...
              <button
                onClick={() => {
                  navigator.clipboard.writeText(`CREATE TABLE IF NOT EXISTS matches (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  date text,
  match_code text,
  league text,

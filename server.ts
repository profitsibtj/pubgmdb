...
const mapDailyStatsFromDb = (row: any) => {
  if (!row) return null;
  return {
    id: String(row.id),
    league: row.league || "",
...
// Daily Stats Endpoints (Player Input Panel entries - separate table from real match results)
app.get("/api/daily-stats", checkAuth, async (req, res) => {
  try {
    const { data: rawRows, error } = await getSupabase().from("daily_stats").select("*");
    if (error) {
      if (error.code === "PGRST205" || error.message?.includes("relation \"daily_stats\" does not exist")) {
        return res.status(409).json({
          error: "DATABASE_SETUP_NEEDED",
          message: "The 'daily_stats' table has not been created in your Supabase database yet."
        });
      }
      throw error;
    }
    const formatted = (rawRows || []).map(r => mapDailyStatsFromDb(r));
    res.json(sortMatches(formatted));
  } catch (error: any) {
...
app.post("/api/daily-stats", checkAdminAuth, async (req, res) => {
  try {
    const dbObj: any = mapDailyStatsToDb(req.body);
    dbObj.created_at = new Date().toISOString();
    const { data, error } = await getSupabase().from("daily_stats").insert([dbObj]).select("id").single();
    if (error) throw error;

    res.json({ success: true, id: String(data.id) });
  } catch (error: any) {
    console.error("Error in POST /api/daily-stats:", error);
    res.status(500).json({ error: error.message });
  }
});

app.put("/api/daily-stats/:id", checkAdminAuth, async (req, res) => {
  try {
    const dbObj: any = mapDailyStatsToDb(req.body);
    dbObj.updated_at = new Date().toISOString();
    const { error } = await getSupabase().from("daily_stats").update(dbObj).eq("id", req.params.id);
    if (error) throw error;

    res.json({ success: true, id: req.params.id });
  } catch (error: any) {
    console.error("Error in PUT /api/daily-stats:", error);
    res.status(500).json({ error: error.message });
  }
});

app.delete("/api/daily-stats/:id", checkAdminAuth, async (req, res) => {
  try {
    const { error } = await getSupabase().from("daily_stats").delete().eq("id", req.params.id);
    if (error) throw error;
    // Reclaim the ID if it was the highest one - see the matches delete route for the same reasoning.
    try {
      await getSupabase().rpc("reset_daily_stats_id_seq");
    } catch (e) {}
    res.json({ success: true });
  } catch (error: any) {
    console.error("Error in DELETE /api/daily-stats:", error);
    res.status(500).json({ error: error.message });
  }
});
...
app.put("/api/tournaments", checkAdminAuth, async (req, res) => {
  try {
    ...
    if (tournaments.length === 0) {
      return res.status(400).json({ error: "At least 1 registered tournament is required." });
    }
